 ...
 Iterable<Method> _parseMethods(ClassElement element) =>
  ...
  Code _generateRequest(MethodElement m, ConstantReader httpMethod) {
   ...
   final options =
    ...
    var returnType = isWrappered 
    ? _getResponseType(wrapperedReturnType) : wrapperedReturnType;
    final bool isWrapperedToResultData = 
    (returnType==null || "void" == returnType.toString())? false: _typeChecker(retrofit.ResultData).isExactlyType(returnType);
    ...
    if (returnType == null || "void" == returnType.toString()) {
      ...
      if (isWrapperedToResultData) {
        ...
        """));
        blocks.add(
          refer("response = await $_dioVar.fetch")
           .call([options])
           .statement,
        );
        blocks.add(Code("""
            ...
            switch (clientAnnotation.parser) {
              ...
              // print('genericArgumentFactories:$genericArgumentFactories');
                ...
                if (typeArgs.length > 0 && genericArgumentFactories) {
                  mapperVal =
                      "final subData = ${_displayString(returnType)}.fromJson(subDataResult,${_getInnerJsonSerializableMapperFn(returnType)});";
                } else {
                  mapperVal =
                      "final subData = ${_displayString(returnType)}.fromJson(subDataResult);";
                }
                ...
                break;
              case retrofit.Parser.DartJsonMapper:
                blocks.add(Code("final subData = JsonMapper.fromMap<${_displayString(returnType)}>(subDataResult);"));
                break;
              default:
                throw ArgumentError('No parser set. Use either MapSerializable, JsonSerializable or DartJsonMapper');
            }
