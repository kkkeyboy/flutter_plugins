...
import 'package:json_model/utils/build_script.dart';
...
class JsonModelRunner {
  ...
  void cleanup() async {
    ...
    if (onlyFile == null) {
      ...
      await BuildScript(['run', (dotSplit..removeLast()).join('.') + '.dart']).build();
    }
  }
  ...
  void warningIfImportNotExists(jsonModel, jsonFile) {
    jsonModel.imports_raw.forEach((importPath) {
      var parentPath = jsonFile.path.substring(0, jsonFile.path.lastIndexOf(path.separator));
      if (!File(path.join(parentPath, '$importPath.json')).existsSync()) {
        print("[Warning] File '$importPath.json' not exist, import attempt on '${jsonFile.path}'");
      }
    });
  }
